...
class SettingsController extends Controller
{
    ...
    public function storeIntake(Request $request)
    {
        $data = $request->validate([
            'label' => ['required', 'string', 'max:100', Rule::unique('crm_intakes', 'label')],
        ], [
    ...
    {
        $data = $request->validate([
            'label' => ['required', 'string', 'max:255', Rule::unique('crm_programmes', 'label')],
        ], [
    ...
    {
        $data = $request->validate([
            'label' => ['required', 'string', 'max:255', Rule::unique('crm_document_types', 'label')],
