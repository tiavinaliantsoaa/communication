...
<div class="mb-6 flex justify-end">
    <a href="{{ route('crm.settings') }}"
       class="inline-flex items-center gap-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 text-sm font-medium px-4 py-2.5 rounded-lg shadow-sm">
...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-6">
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
        ...
        @if(!empty($chartByAdvisor['series']))
            <div id="crm-chart-by-advisor"></div>
        @else
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
        ...
        @if(!empty($chartInscritsByAdvisor['series']))
            <div id="crm-chart-inscrits-advisor"></div>
        @else
            <p class="h-72 flex items-center justify-center text-sm text-slate-400">Aucune donnée</p>
        @endif
    </div>
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
        ...
        @if(!empty($chartByProgramme['series']))
            <div id="crm-chart-by-programme"></div>
        @else
            <p class="h-72 flex items-center justify-center text-sm text-slate-400">Aucune donnée</p>
        @endif
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    new ApexCharts(document.querySelector('#crm-chart-status'), {
        ...
        colors: ['#64748b','#2563eb','#8b5cf6','#15803d'],
    ...
    const horizontalBar = (el, labels, series, color) => {
        ...
        const height = Math.max(288, labels.length * 28 + 48);
        new ApexCharts(el, {
    ...
    horizontalBar(
        document.querySelector('#crm-chart-by-advisor'),
        @json($chartByAdvisor['labels']),
        @json($chartByAdvisor['series']),
        '#1e40af'
    );
    horizontalBar(
